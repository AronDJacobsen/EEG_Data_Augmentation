...
def tableResults(pickle_path, windows_OS, experiment_name, measure="sensitivity", merged_file=False, windowsOS=False):
    ...
    for ratio in results_all:
        ...
        for idx_art, artifact in enumerate(artifacts):
            ...
            for idx_mod, model in enumerate(models):
                ...
                temp_f1 = []
                for fold in folds:
                    store_scores.append(results[fold][artifact][model][measure])
...
def plotPerformanceModels(performance_dict, error_dict, experiment, model_names, artifact_names, ratio, save_img=False,
                          windowsOS=False):
    if windowsOS:
        slash = "\\"
    else:
...
def plotPerformanceClasses(performance_dict, error_dict, experiment, model_names, artifact_names, ratio, save_img=False,
                           windowsOS=False):
    if windowsOS:
        slash = "\\"
    else:
...
def plotResultsSMOTE(performance_list, errors_list, experiment, model_names, artifact_names, SMOTE_ratios,
                     measure="sensitivity", save_img=False, windowsOS=False):
    if windowsOS:
        slash = "\\"
    else:
        slash = "/"
    ...
    for indv_art, name in enumerate(artifact_names):
        ...
        plt.ylim(0, 1)
        plt.title("{:s} scores across models and SMOTEs on the '{:s}'-class".format(measure, name))
        plt.xlabel("Model")
        plt.ylabel(measure)
        ...
        if save_img:
            try:
                os.makedirs(save_path + slash + measure)
                print("New directory created!")
            except FileExistsError:
                pass
...
def getResultsChooseMeasure(pickle_path, windows_OS, experiment_name, merged_file, windowsOS=False,
    ...
    performance_list, errors_list, model_names, artifact_names, SMOTE_ratios = tableResults(measure=measure,
    ...
    plotResultsSMOTE(performance_list=performance_list, errors_list=errors_list, experiment=experiment,
                     ...
                     save_img=save_img,
                     windowsOS=windowsOS, measure=measure)
...
if __name__ == '__main__':
    ...
    experiment = "smote_f2"
    experiment_name = '_smote_LR_f2'
    experiment_name_merge = 'first_merge_smote'
    ...
    measures = ["sensitivity", "accuracy", "weighted_F1"]
    save_img = False

    for measure in measures:

        getResultsChooseMeasure(measure=measure,
                                pickle_path=pickle_path,
                                windows_OS=windowsOS,
                                experiment_name=experiment_name,
                                merged_file=merged_file,
